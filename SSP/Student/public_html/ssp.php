...
$loginQuery = "select * from students WHERE students.username='".$usr."'";
$loginResponse = @mysqli_query($dbc, $loginQuery);

if($loginResponse){
$loginResults = mysqli_fetch_assoc($loginResponse);
//echo '<pre>'; print_r($loginResults); echo '</pre>';
...
//If ssp is marked as LOCKED, display alert, and redirect to login.
...
//If yes, check if previous session started less than 45 minutes ago, and, if so, give countdown and redirect to login. 
...
if ($existingsession >= $currenttime-2700) {
	...
	$anotherdevice = 'This session is currently active on another device or was not properly logged out.  Please return to the other device or wait '.$minutesseconds.' minute(s) before logging in again.';
	   $_SESSION['error'] = $anotherdevice;
	   header("Location: index.php");
	   exit;
...
//If state is marked as MENTOR, display alert, and redirect to login if session started more than 45 minutes ago.
} else if ($sessionCheck[isActive] == 'MENTOR'){
	//If less than 90 minutes, redirect to login 
    $currenttime = date('Y-m-d H:i:s');
	$existingsession = $sessionCheck[startTime];
    if ($existingsession >= $currenttime-5400) {
   		$active = 'Your Student Success Plan is currently being edited by your Mentor.  Please wait a few minutes, or go and speak with your Mentor.';

	$_SESSION['error'] = $active;
	header("Location: index.php");
		   exit;
//If yes, but previous session started longer than 90 minutes ago, go ahead and proceed as normal.
} else {
	goto accepted;
}
}
...
//Set session active.
$setSessionQuery = "update sessions set sessions.isActive='YES' where local_id = ".$localID.";";
...
<div id="main">
	<div id="header">
		...
		<div id="identifiers">	
			<div id="studentname">
				<span>Welcome, '.$loginResults['first_name'].' '.$loginResults['last_name'].'!</span>
			</div>
			<div id="mentor">
				<span>Your mentor teacher is '.$loginResults['mentor'].'.</span>
			</div>
			<div id="classof">
				<span>Class of: '.$loginResults['class_of'].'</span>
			</div>
			<div id="lastupdated">
				<span>Last Updated On: '.$loginResults['last_updated'].'</span>
			</div>
		</div>
		<div id="logout">
...
//Begin Title and Instructions
			echo '
			<div id="plan">';
			echo '<h1>My Student Success Plan</h1>
